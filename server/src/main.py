...
from schemas import DateBatchRequestSchema, DateBatchResponseSchema, DateOperationSchema, DatesByUserSchema
...
async def get_dates_by_user(token: Annotated[str, Depends(requires_auth)]) -> DatesByUserSchema:
    ...
    try:
        dates = await dp.get_dates_by_user(user_id)
    except Exception as err:
        return DatesByUserSchema(ok=False, message=repr(err))
    return DatesByUserSchema(ok=True, dates=dates)
